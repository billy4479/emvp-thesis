...
fn compare_case<const MODULUS: u32, const N: usize>(c: &mut Criterion) {
    ...
    let rhs = dynamic.elements(&common::array_values::<MODULUS, N>(193));
...
fn configure(group: &mut BenchmarkGroup<'_, WallTime>, length: usize) {
    common::tune_group(group, 20, Duration::from_secs(1), Duration::from_secs(3));
    group.throughput(Throughput::Elements(length as u64));
}

fn compare(c: &mut Criterion) {
    if common::skip_in_quick_mode("compare_static_dynamic_ntt") {
        return;
    }
